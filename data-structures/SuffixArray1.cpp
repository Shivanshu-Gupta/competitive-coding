/*
Ref: http://e-maxx-eng.appspot.com/string/suffix-array.html
...
int suffixArray1Test() {
    ...
    for(int cnt = 1, stp = 1; cnt < N; cnt *= 2, ++stp) {
        ...
        for(int i = 1, currRank = 0; i < N; ++i) {
            ...
            suffixRank[stp][L[i].origIdx] = currRank;
        }
    ...
    for(int i = 0; i < N; ++i) cout << L[i].origIdx << '\t' << s.substr(L[i].origIdx) << endl;
